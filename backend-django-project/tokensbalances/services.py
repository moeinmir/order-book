...
class TokenBalanceService:
    ...
    @staticmethod
    def transfer_from_user_hd_wallet_to_central_hd_wallet(token_id, user_id, charging_amount):
    ...
    def transfer_from_user_credit_to_user_hd_wallet(token_id, user_id, amount_to_be_transferred):
        central_hd_wallet_address = settings.CENTRAL_HD_WALLET_ADDRESS
        central_hd_wallet_index = settings.CENTRAL_HD_WALLET_INDEX
        logger.info(f'token_id:{token_id},user_id:{user_id},amount_to_be_transferred:{amount_to_be_transferred}')
        token = TokenBalanceService.get_token_by_id(token_id)
        logger.info(f'token:{token}')
        user = UserService.get_user_by_id(user_id)
        logger.info(f'user:{user}')
        ...
        with transaction.atomic():
            if not account_balance.is_locked:
                ...
                account_balance.is_locked = True
                account_balance.save()
            if(account_balance.free_amount>amount_to_be_transferred):
                logger.info('have enough balance')
                tx = transfer_token(central_hd_wallet_address,user_address, amount_to_be_transferred,token.address,token.type,2)       
                account_balance.hd_wallet_balance = account_balance.hd_wallet_balance + amount_to_be_transferred
                account_balance.free_amount = account_balance.free_amount - amount_to_be_transferred  
                account_balance.is_locked = False 
                account_balance.save()
                return (True, account_balance,tx)
            else:
